...
if __name__ == '__main__':
    ...
    # ******************************************************************
    number = 2943   # 该副图片img的编号
    src_img = X_test[number]    # img的名称/路径
    print('预测图片{}的分类是:'.format(src_img)+str(predictions_labels[number]))
    families = []   # 找到与img同一分类下的所有图片
    for i in range(0, len(X_test)):
        if predictions_labels[i] == predictions_labels[number]:
            families.append(i)
    families_names = []  # 保存同一分类所有图片的名称/路径
    for i in families:
